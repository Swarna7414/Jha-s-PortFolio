...
import '../App.css';
import { startBackgroundBallsEffect } from '../Styles/effect';
...
const Develop: React.FC = () => {
  ...
  return (
    <section className="relative w-screen min-h-screen overflow-hidden">
        <div id="bg-effect" className="absolute inset-0" />
          <div className="circles-bg">
          {Array.from({ length: 20 }).map((_, i) => <span key={i}></span>)}
          </div>
        
      <section className="relative z-10 min-h-screen flex flex-col xl:flex-row scrollbar-hide pt-[350px]">
        <div className="xl:w-1/2 w-full xl:h-screen rounded-3xl xl:mt-4">
          <img
            src={smallscreen}
            alt="Profile Small"
            className="block xl:hidden w-60 sm:w-64 mx-auto mt-10"
          />
          <img
            src={BigScreenRe}
            alt="Profile Large"
            className="hidden xl:block w-full max-h-[90vh] object-cover shadow-sm shadow-emerald-100 rounded-2xl mt-4"
          />
        </div>
